          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#334155]">
              {decisions.map((d) => {
                ...
                const pnlColor =
                  hasPnl && d.pnl! >= 0 ? 'text-green-400' : 'text-red-400'

                return (
                  <tr key={d.id} className="hover:bg-[#334155]/40 transition-colors">
                    <td className="px-4 py-2.5 text-[#94a3b8] font-mono text-xs whitespace-nowrap">
                      {formatTime(d.ts)}
                    </td>
                    <td className="px-4 py-2.5 font-mono font-semibold text-[#f1f5f9]">
                      {d.symbol}
                    </td>
                    <td className="px-4 py-2.5">{actionBadge(d.action)}</td>
                    <td className="px-4 py-2.5 text-right font-mono text-[#f1f5f9]">
                      ${d.price.toFixed(2)}
                    </td>
                    <td className="px-4 py-2.5 text-right text-[#f1f5f9]">{d.qty}</td>
                    <td className="px-4 py-2.5 text-right">
                      <span className="text-[#94a3b8]">{d.confidence}/10</span>
                    </td>
                    <td
                    ...
                    <td className="px-4 py-2.5 text-[#94a3b8] max-w-xs truncate">
                      {d.reasoning}
                    </td>
                  </tr>
                )
